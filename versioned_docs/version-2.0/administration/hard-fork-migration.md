...
slug: "hard-fork-migration"
sidebar_position: 13
---

# Hard-fork migration (from pre-2026-07 Forgente / Gitea-compat deployments)

Forgente completed its hard-fork cutover from its Gitea lineage in July 2026.
Runtime surfaces that previously kept Gitea-compatible names for compatibility
are now Forgente-named. This guide maps every change for operators upgrading
an existing instance across the cutover. Most renames ship with a
...
If you are setting up a brand-new instance, you can ignore this page — it
only matters for deployments that predate the cutover.
...
[build-source.md](https://github.com/forgente/forgente/blob/main/docs/build-source.md)
in the main Forgente repository).
...
a pick-upstream helper in the
[main Forgente repository](https://github.com/forgente/forgente). The API
surface intentionally remains Gitea-compatible, so existing integrations,
SDKs, `tea`, and `act_runner` continue to work.
