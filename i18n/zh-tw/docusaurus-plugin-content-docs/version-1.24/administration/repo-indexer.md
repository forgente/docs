...
通過在您的 [`app.ini`](../administration/config-cheat-sheet.md) 中启用此功能，Gitea 可以通過存放庫的文件進行搜索：
...
Gitea 使用 [`gobwas/glob` 库](https://github.com/gobwas/glob) 中的 glob 模式匹配来選择要包含在索引中的文件。

限制文件列表可以防止索引被派生或無关的文件（例如 lss、sym、map 等）污染，从而使搜索结果更相关。这還有助于减小索引的大小。

`REPO_INDEXER_EXCLUDE_VENDORED`（默认值為 true）将排除供應商文件不包含在索引中。

`REPO_INDEXER_INCLUDE`（默认值為空）是一个逗号分隔的 glob 模式列表，用于在索引中**包含**的文件。空列表表示“_包含所有文件_”。
`REPO_INDEXER_EXCLUDE`（默认值為空）是一个逗号分隔的 glob 模式列表，用于从索引中**排除**的文件。与該列表匹配的文件将不会被索引。`REPO_INDEXER_EXCLUDE` 优先于 `REPO_INDEXER_INCLUDE`。
...
- 所有文件和模式都规范化為小写，因此 `**Makefile`、`**makefile` 和 `**MAKEFILE` 是等效的。
