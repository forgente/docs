...
- [Matrix](https://matrix.to/#/#gitea-space:matrix.org)
  - 注意：大多数 Matrix 频道都与 Discord 中的对應频道桥接，可能在桥接過程中会出現一定程度的不稳定性。
- 中文支持
...
如果您发現了 Bug，請在 GitHub 上 [建立一个问题](https://github.com/go-gitea/gitea/issues)。

**注意：** 在請求支持时，可能需要准备以下信息，以便帮助者获得所需的所有信息：

1. 您的 `app.ini`（将任何敏感数据進行必要的清除）。
2. 您看到的任何错误消息。
...
4. 可重現的步骤，以便他人能够更快速、更容易地重現和理解问题。
   - [demo.gitea.com](https://demo.gitea.com) 可用于重現问题。
5. 如果遇到慢速/挂起/死锁等问题，請在出現问题时报告堆栈跟踪。
...
然后您将在 Gitea 的日志目錄中找到 `*.log` 文件（默认為 `%(GITEA_WORK_DIR)/log`）。

```ini
; 要显示所有 SQL 日志，您還可以在 [database] 部分中设置 LOG_SQL=true
[log]
LEVEL=debug
MODE=console,file
```
...
如果 Web UI 停止工作，您可以尝试通過命令行收集堆栈跟踪：
...
3. 尝试触发 bug，当請求卡住一段时间，使用或浏览器访问：获取堆栈跟踪。
