...
此令牌應該出現在 `To` 和 `Delivered-To` 头字段中。
...
該地址需要包含 `%{token}` 占位符，該占位符将被替换為描述使用者/操作的令牌。
此占位符在地址中只能出現一次，並且必須位于地址的使用者部分（`@` 之前）。

使用电子邮件子地址的示例可能如下：`incoming+%{token}@example.com`

如果使用 catch-all 邮箱，则占位符可以出現在地址的使用者部分的任何位置：`incoming+%{token}@example.com`、`incoming_%{token}@example.com`、`%{token}@example.com`
